    ...
    <DashboardLayout title="Knowledge Graph & RAG">
      <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: "0.8rem", color: "#475569" }}>Neo4j Graph Database • Qdrant Vector Store • LlamaIndex RAG Pipeline</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "14px" }}>
          {[
            ...
            <div key={s.label} style={{ background: "rgba(8,14,26,0.7)", backdropFilter: "blur(16px)", border: `1px solid ${s.color}18`, borderRadius: "14px", padding: "16px 20px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: "1.6rem", fontWeight: 900, color: "#f0f6ff" }}>{s.value}</span>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: "16px" }}>
          ...
          <div style={{ background: "rgba(8,14,26,0.7)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "16px", padding: "20px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
              <h3 style={{ margin: 0, fontSize: "0.85rem", fontWeight: 800, color: "#f0f6ff" }}>Graph Explorer</h3>
              <input placeholder="Search nodes..." style={{ padding: "6px 12px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "8px", color: "#f0f6ff", fontSize: "0.78rem", outline: "none", width: "200px", fontFamily: "inherit" }} />
            </div>
            {/* Visual node layout */}
            <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", justifyContent: "center", padding: "20px 10px", minHeight: "260px", background: "rgba(0,0,0,0.2)", borderRadius: "12px", border: "1px solid rgba(255,255,255,0.04)" }}>
              {NODES.map(node => (
                <div key={node.label} style={{
                  ...
                  <span style={{ fontSize: "0.7rem", fontWeight: 700, color: "#f0f6ff", textAlign: "center" }}>{node.label}</span>
                  <span style={{ fontSize: "0.58rem", color: "#475569", fontWeight: 600 }}>{node.type}</span>
                </div>
              ))}
            </div>
            {/* Legend */}
            <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", marginTop: "14px", paddingTop: "12px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            <div style={{ background: "rgba(8,14,26,0.7)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "16px", padding: "16px" }}>
              <h3 style={{ margin: "0 0 12px", fontSize: "0.82rem", fontWeight: 800, color: "#f0f6ff" }}>🔗 Active Relationships</h3>
              {RELATIONSHIPS.map((r, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: "6px", padding: "6px 0", borderBottom: i < RELATIONSHIPS.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                  <span style={{ fontSize: "0.7rem", fontWeight: 600, color: "#94a3b8", flex: "0 0 130px", textOverflow: "ellipsis", overflow: "hidden", whiteSpace: "nowrap" }}>{r.from}</span>
                  ...
                  <span style={{ fontSize: "0.7rem", fontWeight: 600, color: "#94a3b8" }}>{r.to}</span>
                </div>
              ))}
            </div>
            ...
            {/* Indexed Documents */}
            <div style={{ background: "rgba(8,14,26,0.7)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "16px", padding: "16px" }}>
              <h3 style={{ margin: "0 0 12px", fontSize: "0.82rem", fontWeight: 800, color: "#f0f6ff" }}>📄 RAG Document Store</h3>
              {DOCS.map((doc, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < DOCS.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                  <div>
                    <p style={{ margin: 0, fontSize: "0.75rem", fontWeight: 600, color: "#cbd5e1" }}>{doc.title}</p>
                    <p style={{ margin: "2px 0 0", fontSize: "0.62rem", color: "#475569" }}>{doc.type} • {doc.pages} pages</p>
                  </div>
