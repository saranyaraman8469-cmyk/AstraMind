...
function ThemeBtn() {
  ...
  return (
    <button onClick={() => setTheme(theme === "dark" ? "light" : "dark")} title="Toggle theme"
      style={{ width: 36, height: 36, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.1)", background: "var(--border-subtle)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px" }}>
      {theme === "dark" ? "☀️" : "🌙"}
    </button>
  );
}
...
export default function DashboardLayout({ children, title }: { children: React.ReactNode; title?: string }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg-base)", color: "var(--text-primary)", fontFamily: "'Inter', -apple-system, sans-serif", display: "flex" }}>
      {/* Grid Background */}
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, backgroundImage: "linear-gradient(var(--grid-line) 1px, transparent 1px), linear-gradient(90deg, var(--grid-line) 1px, transparent 1px)", backgroundSize: "60px 60px" }} />
      ...
      {/* SIDEBAR */}
      <div style={{ width: "64px", flexShrink: 0, height: "100vh", position: "sticky", top: 0, background: "var(--bg-panel)", backdropFilter: "blur(20px)", borderRight: "1px solid var(--divider)", display: "flex", flexDirection: "column", alignItems: "center", padding: "16px 0", gap: "4px", zIndex: 100 }}>
        {/* Logo */}
      ...
      <div style={{ flex: 1, minHeight: "100vh", overflow: "auto", position: "relative", zIndex: 10 }}>
        {/* TOPBAR */}
        <div style={{ height: "60px", position: "sticky", top: 0, zIndex: 50, background: "rgba(4,8,15,0.92)", backdropFilter: "blur(20px)", borderBottom: "1px solid var(--divider)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <span style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>{title || "Dashboard"}</span>
          </div>
